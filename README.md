...
- **進階篩選：** 搜尋欄旁的篩選按鈕可依電影／電視劇類型多選（`?genre=28` 或 `?genre=28,35`）。有選類型時改查該類型熱門作品，標題關鍵字暫不套用。
...
- **進階篩選：** 點搜尋欄旁的圖示展開類型標籤，可多選。列表改為該類型目前較熱門的作品（TMDB Discover）。再點一次可取消；「清除篩選」會拿掉所有類型。有篩選時標題搜尋不會送出。
...
- 缺少單元測試。
- TMDB 無法在同一請求同時做標題搜尋與類型篩選；有選類型時以該類型熱門為準。
